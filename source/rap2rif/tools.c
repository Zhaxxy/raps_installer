...
{
	snprintf(ptr, 256, "/dev_hdd0/game/RAPZHAXXY/USRDIR/keys");
	return 0;
}
...
struct keylist *keys_get(enum sce_key type, uint64_t idps[2])
{
...
struct actdat *actdat_get(char *exdata_folder_output) {
	char base[256];
	char path[1056];
	...
	while (1) {
		...
		sprintf(path,"/dev_hdd0/home/%08d/exdata/act.dat",user_id);
	...
	sprintf(exdata_folder_output,"/dev_hdd0/home/%08d/exdata/",user_id);
	return actdat;
